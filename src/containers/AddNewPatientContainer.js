...
const AddNewPatientContainer = (props) => {
  ...
  if (getQuarantineTypes !== undefined && getQuarantineTypes.isLoading) {
    return <LoadingComponent isLoading={true} style={loadingComponentStyle} />;
  }

  if ((getQuarantineTypes !== undefined && getQuarantineTypes.allZonesError) || !getQuarantineTypes) {
    return (
  ...
  return (
    <div style={{ height: '100%' }}>
      <AddNewPatientComponent onSubmit={handlePreview} onCancel={onCancel} types={getQuarantineTypes.types} />
      {showPreview && (
